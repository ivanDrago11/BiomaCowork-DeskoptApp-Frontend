...
function EnhancedTableToolbar(props) {
  const { numSelected, onIconEdit, onIconDelete, information } = props;
...
export function MaterialTableUsers() {
  ...
  const [informationTable, setInformationTable] = React.useState({});
  
  const { users, startLoadingUser, changeIsEditing, loadUser, startDeletingUser} = useUserStore();
  const { isUserModalOpen, closeUserModal, openUserModal, page } = useUiStore();
    ...
    <Box sx={{ width: '100%' }}>
      <Paper sx={{ width: '100%', mb: 2 }}>
        <EnhancedTableToolbar 
            ...
            onIconEdit={() => onOpenEditModal(userId)} 
            onIconDelete={() => onDeleteButton(userId)} 
        ...
        <TablePagination
          ...
          page={tablePage}
          onPageChange={ (event,tablePage) => handleChangePage(event, tablePage, users) }
